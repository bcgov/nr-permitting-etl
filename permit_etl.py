...
# permit_etl.py
# ---------------------------------------------------------------------------
# Transform permit source data into **ProcessEventSet**
...
# Remove the following line if you run it in FME or other ETL tool.
# Remove the following line if you run it in FME or other ETL tool.
# Remove the following line if you run it in FME or other ETL tool.
# Remove the following line if you run it in FME or other ETL tool.
# Remove the following line if you run it in FME or other ETL tool.
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Permit ETL → ProcessEventSet JSON Lines",
    )
    p.add_argument("--source", default=DEFAULT_SOURCE_FILE,
    ...
    p.add_argument("--rules",  default=DEFAULT_RULES_FILE,
               help="rules.json path")
    p.add_argument("--lifecycle", default=DEFAULT_LIFECYCLE_FILE,
               help="lifecycle_map.json path")
    p.add_argument("--system", help="filter rules by 'source' field")
    p.add_argument("--out",    default=DEFAULT_OUTPUT_DIR,
               help="output directory")

    p.add_argument("--outfile", default=DEFAULT_OUTPUT_FILENAME,
               help="output filename (can end with .gz)")
    ...
    # Tunable thresholds
    p.add_argument("--small-mb", type=int, metavar="N", help="override small")
    p.add_argument("--medium-mb", type=int, metavar="N", help="override medium")
    p.add_argument(
        "--chunk-rows",
        type=int,
        metavar="N",
        help="override pandas chunk row count",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.source or not args.rules or not args.out:
        raise ETLError(
            "Missing required arguments. "
            "You must supply --source, --rules, and --out "
            "or set SOURCE_FILE, RULES_FILE, and OUTPUT_DIR"
        )


    # Fill in from env if not set
    # args.source = args.source or os.getenv("SOURCE_FILE")
    # args.rules = args.rules or os.getenv("RULES_FILE")
    # args.lifecycle = args.lifecycle or os.getenv("LIFECYCLE_FILE")
    # args.out = args.out or os.getenv("OUTPUT_DIR")
    # args.outfile = args.outfile or os.getenv("OUTPUT_FILE") or "events.jsonl"


    # Instantiate config merging ENV + CLI overrides -------------------------
    cfg = Config()
    if args.small_mb:
        cfg.small_mb = args.small_mb
    if args.medium_mb:
        cfg.medium_mb = args.medium_mb
    if args.chunk_rows:
        cfg.chunk_rows = args.chunk_rows
    if args.log_level:
        cfg.log_level = args.log_level

    _init_logging(cfg, verbose=args.verbose)

    try:
        rules = load_rules(Path(args.rules), system=args.system)
        ...
        logger.error("ETL failed: %s", err, extra={"error_code": "ETL_ERROR"})
        sys.exit(1)
...
# In development:
# export $(cat .env | xargs)  # or use `dotenv` or `source .env`
# python permit_etl.py --source permits.csv --rules rules.json --out ./out

# In production or Docker
# env $(cat .env | xargs) python permit_etl.py --source permits.csv --rules rules.json --out ./out
